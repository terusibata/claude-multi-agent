...
- [ ] AppArmor プロファイルが作成され、コンテナに適用されている
- [ ] 全ライフサイクルイベントが統一フォーマットの監査ログとして出力される
- [ ] コンテナクラッシュ時にユーザーへ復旧通知SSEイベントが送信される
- [ ] Proxy クラッシュ時に自動再起動が動作する
- [ ] S3 ライフサイクルポリシーが定義され、適用手順が文書化されている
- [ ] エージェント実行中にツール結果ごとのファイル同期が動作する
- [ ] Node.js プロセスが Proxy 経由で外部通信できる
- [ ] E2E 統合テストが全てパスする
- [ ] 全ペネトレーションテストシナリオが期待通りに防御される
- [ ] 移行ドキュメントが最新状態に更新されている
